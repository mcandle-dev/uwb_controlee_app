...
- **P7.** BLE 광고/GATT 수명 ≠ UWB 세션 수명 (의도적 분리). 세션 자동 실패
  ...
  **예외 — 모드 4 (2026-08-16 개정, 근거 spec 002):** 모드 4(GATT-CLIENT)는 세션 종료 시
  자동 실패라도 **채널·FGS 를 닫고 휴면 복귀**한다. 재발급 전달은 세션 중 "연결 유지 재Write"
  가, 종료 후 재교환은 자동 감시 웨이크(또는 사용자 재Start 시 재연결 ~1초)가 대체하므로
  유지가 불필요하고, 유지하면 콘솔 재광고에 조용히 재연결돼 폰 세션 없이 콘솔만 트리거되는
  반쪽 연결이 생긴다 (실기기 확인 2026-08-16).
- **P8.** FGS(`RangingForegroundService`) 수명 = "OOB 유지 ∪ 세션 활성".
  FGS 없이 백그라운드 진입 = 무증상 세션 종료. FGS 시작은 반드시 포그라운드에서.
  **예외 — 콜드 웨이크 (2026-08-16 개정, 근거 spec 002 T304 실측):** 배터리 최적화 제외
  (power-save exemption) 앱은 백그라운드 FGS 시작이 허용된다 — 자동 감시 웨이크 경로는
  이 조건 위에서만 성립하며, 토글 ON 시 앱이 제외 설정을 요청·안내한다.
