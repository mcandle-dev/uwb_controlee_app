...
- ~~**사양서 §2-1 (Draft)**: 모드 3 의 "자신 OOB_INFO 병행 송출" 여부~~ →
  **2026-08-12 확정: 병행 송출** (사람, 이 리포 세션에서 — tasks.md T002 기록 참조).
  마스터 사양서 Draft 해제·버전 업은 콘솔 세션 몫 (handoff 통보 완료).
- 모드 선택 UI 의 위치(메인 화면 vs 별도 설정) — plan D3 에서 결정 (메인 화면 드롭다운으로 확정).
