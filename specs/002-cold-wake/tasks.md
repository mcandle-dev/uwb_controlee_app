...
- 2026-08-12 (논의) iOS 검토 결과 **방향 재검토 항목이 spec.md 미해결에 추가됨** — iOS 는
  현 광고 포맷으로 콜드 웨이크 불가(UUID 목록 AD 부재 + 백그라운드 nil 스캔 금지)이며,
  Service Data 송출 API 부재로 폰 주소 자동 전달도 불가. 대안 **모드 4(GATT 역방향)** 도출
  → `docs/guide/ble_dev_guide.md` §7 에 제안으로 기록(미채택·계약 미변경).
  **G1 승인 전에 "iOS 지원 여부" 결정 필요** — 결정 전까지 Phase 1~4 착수 보류.
