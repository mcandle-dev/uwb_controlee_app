...
- 2026-08-16 (후속) **간헐 동작 관찰 (사용자)** — 웨이크가 될 때/안 될 때 섞임, 원인 미특정.
  진단 도구 추가: OobWakeReceiver 가 웨이크 시도·결과(FGS 기동/거부/스캔 오류)를 prefs 에
  최근 5건 기록하고, 앱 실행 시 로그 콘솔에 "지난 웨이크 이력"으로 표시 (adb 불필요).
  시나리오 가이드 §8-2 에 진단 매트릭스(스로틀 10초·BT 재시작 시 등록 소멸·Doze 지연·
  잠자는 앱·콘솔 광고 중단(연결 점유)) + 테스트 기록 요령 추가. **추가 실기기 테스트 진행 중
  — T304 최종 마감은 원인 특정 후.**
