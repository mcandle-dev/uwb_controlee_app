...
├─────────────────────────────────────┤
│ 내 UWB 주소  [ 5F:DD ]  (탭=복사)     │ ← PC 스크립트 --dest-mac에 입력할 값
├─────────────────────────────────────┤
│ 보드 MAC [00:00]  Session ID [42]    │ ← 입력 2개, 나머지 파라미터는 상수
│ CH 9 · PRE 9 · DS-TWR · 120ms · STS…│ ← 고정 파라미터 읽기 전용 표기
├─────────────────────────────────────┤
│      [ ▶ Start ]   [ ■ Stop ]       │
│ ① 앱 Start → ② PC 스크립트 실행       │ ← 순서 안내 (controlee 먼저!)
├─────────────────────────────────────┤
...
├── MainActivity.kt        # 진입점. 권한 런처, 설정 화면 이동, onResume마다 가용성 재판정
├── MainViewModel.kt       # UiState(StateFlow) 단일 상태. 상태 머신·워치독·로그·수명주기
├── uwb/
│   ├── UwbDefaults.kt     # ★ 세션 계약 상수 — 보드(PC 스크립트)와 대조하는 파일
...
- **세션 소유**: ViewModel이 레인징 Job을 소유 — 화면 회전에도 세션 유지,
  앱 백그라운드 진입 시엔 `ProcessLifecycleOwner`로 감지해 세션 정지(백그라운드 레인징 미지원).
- **핵심 코드 흐름 (controlee)**:
...
| 보드 | DWM3001CDK + sasodoma 리포 동봉 UCI 펌웨어 + PC(Python 3.8+) |
...
- [ ] 거리 30cm~2m, 장애물 없음, 테스트 내내 앱 포그라운드 유지
- [ ] 안 되면: 가이드 §6 트러블슈팅 순서대로 → 폰 로그 콘솔 + PC 콘솔 출력 확보

## Fork해서 개발할 때 알아야 할 것

1. **Ground truth 문서**: 기능·화면은 `docs/앱_기능_화면_요구사항정의서.md`,
   세션 파라미터·기술 계약은 `CLAUDE.md`가 기준 (충돌 시 CLAUDE.md 우선).
2. **아키텍처 규칙**: UI는 ViewModel(→Repository)에만 의존, 상태는 StateFlow 단방향,
   모든 함수 명시적 타입, 매직넘버 금지(상수화), 한 함수 30줄 이내.
3. **세션 파라미터를 바꿀 때**: 반드시 `UwbDefaults.kt` 한 곳에서 바꾸고, PC 스크립트
   쪽 값과 **쌍으로** 바꿀 것. 한쪽만 바꾸면 에러 없이 조용히 실패합니다.
4. **스코프 추가 금지 항목** (요구사항정의서 §2): BLE OOB 자동 교환, 다중 피어,
   백그라운드 레인징, 측정 저장/내보내기, controller 모드, 설정 화면.
...
6. **진행 상태**: 구현 1~4단계 완료(보드 없이 가능한 전 구간, 실기기 검수 포함).
   5단계(실물 페어)는 보드 확보 대기 중 — 이력은 `docs/작업일지_*.md` 참조.
...
- 짝 프로젝트(PC 레이더 콘솔): `radar_test_console` (별도 리포, 보드 쪽 거리 표시 담당)
