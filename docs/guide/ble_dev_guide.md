...
## 7. 모드 4 — GATT 역방향 (✅ 사양서 v0.5 로 계약 확정)

> **G0 확정 (2026-08-12): Android+iOS 모두 지원, 충돌 시 iOS 기준** → 이 구조가 채택됐고,
> **사양서 v0.5 (§3-1·§5-4·§6·§10)로 계약까지 확정됐다.** 확정값: 서비스 = `5F1D0003`
> (콘솔 방향 식별자 승계), **BOARD_INFO `5F1D0004` (Read)** / **PHONE_INFO `5F1D0005`
> (Write Without Response)**, 콘솔 광고 = Flags + 128-bit UUID 목록 21B `connectable=true`.
> 모드 3 과는 **대체 관계** (동시 운용 안 함). 폰 구현: `uwb/OobCentral.kt` (spec 002 T202).
...
### 7-4. 계약 변경 내역 (✅ 사양서 v0.5 로 반영 완료)

1. ~~Write 특성 신설~~ → **PHONE_INFO `5F1D0005`, Write Without Response 로 확정** (§3-1 —
   iOS 백그라운드 왕복 최소화. 응답이 없어 콘솔의 검증 실패는 폰에 전달되지 않는다, §7-18)
2. ~~콘솔 GATT 서버 신설~~ → 콘솔 spec 009 로 진행 (콘솔 리포)
3. ~~connectable=true~~ → 모드 4 광고 §5-4 로 확정 (모드 3 과 대체 관계)
4. ~~사양서 버전 업~~ → v0.5 양 리포 사본 배치 완료
