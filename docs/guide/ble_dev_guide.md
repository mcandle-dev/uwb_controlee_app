...
| §7 | **모드 4 제안** (미채택 — 계약 아님) | iOS·백그라운드까지 보려 할 때 |
...
## 7. 모드 4 제안 — GATT 역방향 (⚠ 제안 단계 · 미채택 · 계약 아님)

> **이 절은 사양서가 아니다.** 2026-08-12 논의에서 나온 설계 후보를 기록해 둔 것이며,
> 채택하려면 사양서 개정(버전 업 + 양 리포 동시 커밋 + pin bump)이 선행돼야 한다 (P5/P13).
...
`specs/002-cold-wake` 를 이 구조로 재설계하면 Android·iOS 를 한 경로로 묶을 수 있다
(**재검토 대기 중 — 아직 결정되지 않았다**).
...
| **모드 4 (GATT 역방향) 신설** — §7 | 백그라운드까지 포함하면 이쪽이 근본책. 폰이 central 이라 iOS overflow 문제를 피하고, Android·iOS 통합 경로가 된다. **계약 변경 필요 · 미채택 · 재검토 대기** |
...
**아직 결정되지 않았다 — 재검토 대기 중이다.** 현재 `specs/002-cold-wake` 는 Android 전용
PendingIntent 방식으로 작성돼 있고, 착수 게이트(G1 조정자 추출 승인 · G2 spec 001 실기기
검수 통과) 때문에 코드는 0줄이다. 즉 **지금이 방향을 바꾸기에 가장 싼 시점**이다.

판단에 필요한 재료:

| | 현행 002 (Android PendingIntent) | 모드 4 기반 재설계 |
|---|---|---|
| 계약 변경 | 없음 (기존 광고를 그대로 트리거) | **필요** — 양 리포 동시 작업 |
| iOS | 불가 | 가능 (제약은 §7-5) |
| 검증 리스크 | T101(FGS 예외) 미확인 | 위 + iOS 백그라운드 UWB 미확인 |
| 콘솔 작업량 | 없음 | GATT 서버 신설 (별도 세션) |

**iOS 를 지원 범위에 넣을 것인지**가 갈림길이다. Android 전용이면 현행 002 가 더 싸고,
iOS 가 들어오면 모드 4 로 통합하는 편이 결과적으로 싸다.
